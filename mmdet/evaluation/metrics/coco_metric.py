...
class CocoMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        self.metrics = metric if isinstance(metric, list) else [metric]     # metrics=['bbox', 'segm']
        ...
        if iou_thrs is None:
            iou_thrs = np.linspace(     # iou_thrs=[0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95]
                .5, 0.95, int(np.round((0.95 - .5) / .05)) + 1, endpoint=True)
        self.iou_thrs = iou_thrs
        self.metric_items = metric_items    # none
        self.format_only = format_only      # false
        if self.format_only:
        ...
        if ann_file is not None:
            ...
            self._coco_api = None   # set coco_api
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        gts, preds = zip(*results)  # anns of gts contains bbox/mask  predsof gts contains bboxes/scores/labels/masks
        ...
        for metric in self.metrics:    
            ...
            iou_type = 'bbox' if metric == 'proposal' else metric       # iou_type = 'bbox'/'segm'
            ...
            metric_items = self.metric_items        # none
