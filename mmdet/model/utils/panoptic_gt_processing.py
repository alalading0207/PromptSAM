...
def preprocess_panoptic_gt(gt_labels: Tensor, gt_masks: Tensor,
    ...
    things_masks = gt_masks.to_tensor(
        dtype=torch.bool, device=gt_labels.device)      # 转成了bool值

    if gt_semantic_seg is None:
        masks = things_masks.long() # 变成了整型
...
def preprocess_panoptic_softgt(gt_labels: Tensor, gt_masks: Tensor,
                           gt_semantic_seg: Tensor, num_things: int,
                           num_stuff: int) -> Tuple[Tensor, Tensor]:
    """Preprocess the ground truth for a image.    """
    num_classes = num_things + num_stuff
    things_masks = gt_masks.to_tensor(                  # 设备上传
        dtype=torch.float16, device=gt_labels.device)    # 转为float16和预测值保持精度一致
