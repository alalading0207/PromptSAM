...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def __init__(self,
        ...
        if batch_augments is not None:      # 填充数据到固定大小的预处理模块实例化
            self.batch_augments = nn.ModuleList(
                [MODELS.build(aug) for aug in batch_augments])
        else:
            self.batch_augments = None
        self.pad_mask = pad_mask                # true
        self.mask_pad_value = mask_pad_value    # 0
        self.pad_seg = pad_seg                  # false
        self.seg_pad_value = seg_pad_value      # 255
        self.boxtype2tensor = boxtype2tensor
    
    def forward(self, data: dict, training: bool = False) -> dict:  #  基础数据预处理器  (3,465,465)
        ...
        """# data里包括input和data_samples 包括：meta information\data fields(label\mask\bbox)
        batch_pad_shape = self._get_pad_shape(data)  # (1024, 1024)
        data = super().forward(data=data, training=training)   # 调用父类
        inputs, data_samples = data['inputs'], data['data_samples']

        if data_samples is not None:    # 处理data_samples中的元信息
            # NOTE the batched image size information may be useful, e.g.
            # in DETR, this is needed for the construction of masks, which is
            # then used for the transformer_head. 注意：在 DETR 中，构建掩码时就需要批次图像大小用于变换器头。
            batch_input_shape = tuple(inputs[0].size()[-2:])
            for data_sample, pad_shape in zip(data_samples, batch_pad_shape):
                data_sample.set_metainfo({  # 为data_sample添加pad_shape信息
                    'batch_input_shape': batch_input_shape,
                    'pad_shape': pad_shape
                })

            if self.boxtype2tensor:         # true
                samplelist_boxtype2tensor(data_samples)

            if self.pad_mask and training:  # true
                self.pad_gt_masks(data_samples)   # pad input 后 pad mask

            if self.pad_seg and training:   # false
                self.pad_gt_sem_seg(data_samples)

        if training and self.batch_augments is not None:    # batch_augments是一个modulelist(MODELS.build(aug)) 
            for batch_aug in self.batch_augments:   # BatchFixedSizePad()   # 批次增强处理
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
            for ori_input in _batch_inputs:  # torch.Size([3, 465, 465])
                pad_h = int(
                    np.ceil(ori_input.shape[1] /
                            self.pad_size_divisor)) * self.pad_size_divisor   # 480
                pad_w = int(
    ...
    def pad_gt_masks(self,
        ...
        if 'masks' in batch_data_samples[0].gt_instances:   # gt_instances有masks这项时
            for data_samples in batch_data_samples:
                masks = data_samples.gt_instances.masks     # Bitmap包含多个对象掩码
                data_samples.gt_instances.masks = masks.pad(    # 这里加断点
                    data_samples.batch_input_shape,
                    pad_val=self.mask_pad_value)            # 用0 pad到 480，480
