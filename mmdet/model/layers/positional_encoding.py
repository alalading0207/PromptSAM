...
class SinePositionalEncoding(BaseModule):
    ...
    def forward(self, mask: Tensor, input: Optional[Tensor] = None) -> Tensor:
        ...
        if mask is not None:
            B, H, W = mask.size()       # 1,8,8
            device = mask.device
            # For convenience of exporting to ONNX,
            # it's required to convert
            # `masks` from bool to int.
            mask = mask.to(torch.int)   # 全为0
            not_mask = 1 - mask  # logical_not  全为1
            y_embed = not_mask.cumsum(1, dtype=torch.float32)   # [1,1,...,1],[2,..],...,[8,8,...,8]
            x_embed = not_mask.cumsum(2, dtype=torch.float32)   # [1,2,...,8],...,[1,2,...,8]
        else:
        ...
        if self.normalize:
            y_embed = (y_embed + self.offset) / \
                      (y_embed[:, -1:, :] + self.eps) * self.scale      # 缩小
            x_embed = (x_embed + self.offset) / \
                      (x_embed[:, :, -1:] + self.eps) * self.scale
        dim_t = torch.arange(
            self.num_feats, dtype=torch.float32, device=device)      # 0到63
        dim_t = self.temperature**(2 * (dim_t // 2) / self.num_feats)# 放大
        pos_x = x_embed[:, :, :, None] / dim_t      # torch.Size([1, 8, 8, 64])
        pos_y = y_embed[:, :, :, None] / dim_t
        # use `view` instead of `flatten` for dynamically exporting to ONNX

        pos_x = torch.stack(
            (pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4).view(B, H, W, -1)
        pos_y = torch.stack(
            (pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), dim=4).view(B, H, W, -1)
        pos = torch.cat((pos_y, pos_x), dim=3).permute(0, 3, 1, 2)  # torch.Size([1, 128, 8, 8])
        return pos
