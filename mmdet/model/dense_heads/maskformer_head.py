...
from ..utils import multi_apply, preprocess_panoptic_gt, preprocess_panoptic_softgt
...
class MaskFormerHead(AnchorFreeHead):
    ...
    def preprocess_gt(
        ...
        """
        num_things_list = [self.num_things_classes] * len(batch_gt_instances)   # 1
        num_stuff_list = [self.num_stuff_classes] * len(batch_gt_instances)     # 0
        gt_labels_list = [
        ...
        targets = multi_apply(preprocess_panoptic_gt, gt_labels_list,
                              ...
                              num_stuff_list)   # 将函数 preprocess_panoptic_gt 应用于多个输入列表，并将函数的输出结果按照输出类型分组
    ...
    def preprocess_softgt(
            self, batch_gt_instances: InstanceList,
            batch_gt_semantic_segs: List[Optional[PixelData]]) -> InstanceList:
        """Preprocess the ground truth for all images.   use soft masks     """
        num_things_list = [self.num_things_classes] * len(batch_gt_instances)   # 1
        num_stuff_list = [self.num_stuff_classes] * len(batch_gt_instances)     # 0
        gt_labels_list = [
            gt_instances['labels'] for gt_instances in batch_gt_instances
        ...
        targets = multi_apply(preprocess_panoptic_softgt, gt_labels_list,
                              gt_masks_list, gt_semantic_segs, num_things_list,
                              num_stuff_list)   # 将函数 preprocess_panoptic_gt 应用于多个输入列表，并将函数的输出结果按照输出类型分组
        labels, masks = targets
        batch_gt_instances = [
    ...
    def get_targets(
        ...
        res = (labels_list, label_weights_list, mask_targets_list,
               mask_weights_list, avg_factor)       # 构建返回结果
