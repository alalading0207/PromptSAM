...
class MaskFormerFusionHead(BasePanopticFusionHead):
    ...
    def loss(self, **kwargs):
        """MaskFormerFusionHead has no training loss.MaskFormerFusionHead主要用于推理，没有训练损失"""
        return dict()
    ...
    def instance_postprocess(self, mask_cls: Tensor,                # torch.Size([30, 2])
                             mask_pred: Tensor) -> InstanceData:    # torch.Size([30, 224, 224])
        ...
        num_queries = mask_cls.shape[0]     # 30 

        # 对分类的预测进行0-1归一化：shape (num_queries, num_class)
        scores = F.softmax(mask_cls, dim=-1)[:, :-1]

        # 获取高分查询：shape (num_queries * num_class, )
        ...
        # 提取查询和类别：
        labels_per_image = labels[top_indices]   # 通过top_indices从labels中提取对应的类别标签  实际上只有一类就是0
        query_indices = top_indices // self.num_classes     
        mask_pred = mask_pred[query_indices]     # 提取对应的掩码 torch.Size([30, 224, 224]) 实际上没变mask_pred

        # 过滤背景类别：extract things 的 得分、实例、掩码
        is_thing = labels_per_image < self.num_things_classes    # 全true  所以，query数不变
        scores_per_image = scores_per_image[is_thing]            # query个[分类得分]
        labels_per_image = labels_per_image[is_thing]
        mask_pred = mask_pred[is_thing]         # 没有忽略值所以 实际上没变mask_pred

        # 掩码二值化
        mask_pred_binary = (mask_pred > 0).float()   

        # 对每个query，掩码中每个像素的概率值 乘以 二值化掩码，再对掩码区域的像素取平均（除以了掩码区域像素数）
        mask_scores_per_image = (mask_pred.sigmoid() *           
                                    mask_pred_binary).flatten(1).sum(1) / (
                                        mask_pred_binary.flatten(1).sum(1) + 1e-6)  # query个mask分数 有的分数可能为0  torch.Size([30])
        
        
        # 结合[分类得分scores_per_image]和[掩码得分mask_scores_per_image]   torch.Size([30])
        det_scores = scores_per_image * mask_scores_per_image   
        mask_pred_binary = mask_pred_binary.bool()
        bboxes = mask2bbox(mask_pred_binary)

        results = InstanceData()
        results.bboxes = bboxes                 # torch.Size([30, 4])
        results.labels = labels_per_image       # 全0 因为只有一类
        results.scores = det_scores             # 结合[分类得分scores_per_image]和[掩码得分mask_scores_per_image]
        results.masks = mask_pred_binary
        return results
    

    def instance_postprocess_large(self, mask_cls: Tensor,
                             mask_pred: Tensor) -> InstanceData:
        
        max_per_image = self.test_cfg.get('max_per_image', 100)
        num_queries = mask_cls.shape[0]
        # shape (num_queries, num_class)
        scores = F.softmax(mask_cls, dim=-1)[:, :-1]        # 对分类的预测进行0-1归一化 1.2931e-07 ~ 3.4810e-06  从30，2 到30，1
        # shape (num_queries * num_class, )
        labels = torch.arange(self.num_classes, device=mask_cls.device).\
            unsqueeze(0).repeat(num_queries, 1).flatten(0, 1)
        scores_per_image, top_indices = scores.flatten(0, 1).topk(
            max_per_image, sorted=False)         # top_indices是得分最高的索引
        labels_per_image = labels[top_indices]   # 通过top_indices从labels中提取对应的类别标签  实际上只有一类就是0

        query_indices = top_indices // self.num_classes     
        mask_pred = mask_pred[query_indices]     # 提取对应的掩码

        # extract things 的 得分、实例、掩码
        is_thing = labels_per_image < self.num_things_classes    # 全true  所以，query数不变
        scores_per_image = scores_per_image[is_thing]            # query个分类得分
        labels_per_image = labels_per_image[is_thing]
        mask_pred = mask_pred[is_thing]

        mask_pred_binary = (mask_pred > 0).float()               # 掩码二值化
        mask_scores_per_image = (mask_pred.sigmoid() *           # 对每个query，掩码中每个像素的概率值 乘以 二值化掩码，再对掩码区域的像素取平均（除以了掩码区域像素数）
                                    mask_pred_binary).flatten(1).sum(1) / (
                                        mask_pred_binary.flatten(1).sum(1) + 1e-6)  # query个mask分数 有的分数可能为0
        det_scores = scores_per_image * mask_scores_per_image   # 结合分类得分和掩码得分
        mask_pred_binary = mask_pred_binary.bool()
        bboxes = mask2bbox(mask_pred_binary)
        ...
        results.masks = mask_pred  # 不传递mask_pred_binary  传递浮点型   这是不一样的的地方
        return results
