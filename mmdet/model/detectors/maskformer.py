...
class MaskFormer(SingleStageDetector):
    ...
    def __init__(self,
        ...
        # backbone: MMPretrainSamVisionEncoder里的vision_encoder
        self.backbone = MODELS.build(backbone)   
        
        # neck: 特征金字塔  RSSimpleFPN 用于将特征放大成多个尺度
        if neck is not None:
            self.neck = MODELS.build(neck)       

        # panoptic_head：全景头
        panoptic_head_ = panoptic_head.deepcopy()
        panoptic_head_.update(train_cfg=train_cfg)  # 添加了train_cfg  在基础配置文件中
        panoptic_head_.update(test_cfg=test_cfg)
        self.panoptic_head = MODELS.build(panoptic_head_)   # 进RSMask2FormerHead

        # panoptic_fusion_head：全景融合头
        panoptic_fusion_head_ = panoptic_fusion_head.deepcopy()
        panoptic_fusion_head_.update(test_cfg=test_cfg)
        self.panoptic_fusion_head = MODELS.build(panoptic_fusion_head_)     # RSMaskFormerFusionHead()
...
@MODELS.register_module()
class MaskFormer_adapt(SingleStageDetector):
    def __init__(self,
                 backbone: ConfigType,
                 neck: OptConfigType = None,
                 panoptic_head: OptConfigType = None,
        ...
        panoptic_head_.update(train_cfg=train_cfg)  # 添加了train_cfg  在基础配置文件中
        panoptic_head_.update(test_cfg=test_cfg)
        panoptic_head_.update(stage=stage)          # 添加stage
        self.panoptic_head = MODELS.build(panoptic_head_)   # 进RSMask2FormerHead

        # panoptic_fusion_head：全景融合头
        panoptic_fusion_head_ = panoptic_fusion_head.deepcopy()
        panoptic_fusion_head_.update(test_cfg=test_cfg)
        panoptic_fusion_head_.update(stage=stage)   # 添加stage
        self.panoptic_fusion_head = MODELS.build(panoptic_fusion_head_)     # RSMaskFormerFusionHead()
