...
class ClassificationCost(BaseMatchCost):
    ...
    def __call__(self,
        ...
        Returns:
            Tensor: Match Cost matrix of shape (num_preds, num_gts).
        """
        pred_scores = pred_instances.scores     # [30,2]
        gt_labels = gt_instances.labels         # 5
...
class CrossEntropyLossCost(BaseMatchCost):
    ...
    def _binary_cross_entropy(self, cls_pred: Tensor,
        ...
        """
        cls_pred = cls_pred.flatten(1).float()      # torch.Size([30, 12544])
        gt_labels = gt_labels.flatten(1).float()    # torch.Size([5, 12544])
        n = cls_pred.shape[1]
        pos = F.binary_cross_entropy_with_logits(
            cls_pred, torch.ones_like(cls_pred), reduction='none')      # torch.Size([30, 12544])
        neg = F.binary_cross_entropy_with_logits(
            cls_pred, torch.zeros_like(cls_pred), reduction='none')     # torch.Size([30, 12544])
        cls_cost = torch.einsum('nc,mc->nm', pos, gt_labels) + \
            torch.einsum('nc,mc->nm', neg, 1 - gt_labels)               
        cls_cost = cls_cost / n                                         # torch.Size([30, 5])
    ...
    def __call__(self,
        ...
        Returns:
            Tensor: Match Cost matrix of shape (num_preds, num_gts).
        """
        pred_masks = pred_instances.masks       # torch.Size([30, 12544])
        gt_masks = gt_instances.masks           # torch.Size([5, 12544])
        if self.use_sigmoid:
