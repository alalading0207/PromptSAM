...
class HungarianAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        num_gts, num_preds = len(gt_instances), len(pred_instances) # 5, 30
        ...
        cost_list = []
        for match_cost in self.match_costs:     # match_costs:class \crossentroy \dice   cost
            cost = match_cost(
                pred_instances=pred_instances,
                gt_instances=gt_instances,
                img_meta=img_meta)
            cost_list.append(cost)
        cost = torch.stack(cost_list).sum(dim=0)    # torch.Size([30, 5])
        ...
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost)    # 5,6,12,18,26 / 3,4,2,0,1
