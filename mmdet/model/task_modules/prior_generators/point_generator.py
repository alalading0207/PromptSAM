...
class MlvlPointGenerator:
    ...
    points-based detectors.二维多级（Mlvl）特征图的标准点生成器基于点的检测器
    ...
    def single_level_grid_priors(self,
        ...
        Return:
            ...
            (coord_x, coord_y, stride_w, stride_h).
        """
        feat_h, feat_w = featmap_size       # 8,8
        stride_w, stride_h = self.strides[level_idx]        # level_idx=4 时 stride_w=64
        shift_x = (torch.arange(0, feat_w, device=device) +
                   self.offset) * stride_w      # 序列长度为8  32，96，160，...，480 中间都差64
        ...
        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)   # [64]  让shift_x复制8次并连接为shift_xx
        if not with_stride:
            shifts = torch.stack([shift_xx, shift_yy], dim=-1)  # [64, 2]   ：[32,32],..,[480,32],  [32,96],...,[480,96], ......[480,480]
        else:
            ...
            shifts = torch.stack([shift_xx, shift_yy, stride_w, stride_h],
                                 dim=-1)
        all_points = shifts.to(device)      # [64, 2]   生成了64个点
        return all_points
