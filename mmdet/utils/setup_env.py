...
def setup_cache_size_limit_of_dynamo():
    ...
    注意：由于对象检测算法中的损失计算和后处理部分是动态的，因此每次运行时都必须编译这些函数。
    为 torch._dynamo.config.cache_size_limit 设置较大的值可能会导致重复编译，从而降低训练和测试速度。
    因此，我们需要将 cache_size_limit 的默认值设置得小一些。经验值为 4。
    """
