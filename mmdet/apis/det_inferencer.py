...
class DetInferencer(BaseInferencer):
    ...
    def _inputs_to_list(self, inputs: InputsType) -> list:
        ...
        if isinstance(inputs, str):
            backend = get_file_backend(inputs)      # 'LocalBackend'
...
class DetInferencer_adapt(BaseInferencer):
    """source-target
    """
    preprocess_kwargs: set = set()
    forward_kwargs: set = set()
    ...
    def __init__(self,
        ...
        if isinstance(model, str):
            ...
            self.stage = cfg.get('stage', None)
            self.prototype_savepath = cfg.get('prototype_savepath', None)
        else:
            self.stage = None
        self.num_visualized_imgs = 0
        self.num_predicted_imgs = 0
    ...
    def __call__(
        ...
        if self.stage == 'compute_prototype':
            ...
            torch.save(self.model.objective_vectors, osp.join(out_dir,'prototype.pth'))  
        return results_dict
        ...
        if preds == None:
            ...
            for single_input, pred in zip(inputs, preds):
                ...
                if self.stage == 'get_softlabel':
                    if img_out_dir != '':
                        vis_dir = osp.join(img_out_dir, 'vis')
                        os.makedirs(vis_dir, exist_ok=True)
                        # soft type npy
                        out_file = osp.join(vis_dir, img_name.replace('.tif', '.npy'))    
                    else:
                        out_file = None
                else:
                    out_file = osp.join(img_out_dir, 'vis',img_name) if img_out_dir != '' else None

                self.visualizer.add_datasample_adapt(       # DetLocalVisualizer add_datasample_adapt
                    img_name,
                    img,
                    pred,
                    show=show,
        ...
        **kwargs,
        ...
        if preds == None:
            ...
            if not return_datasamples:
                results = []
                for pred in preds:
                    result = self.pred2dict(pred, pred_out_dir)     # pred to json
                    results.append(result)
            elif pred_out_dir != '':
                warnings.warn('Currently does not support saving datasample '
                            'when return_datasamples is set to True. '
...
class DetInferencer_large(DetInferencer):

    def preprocess(self, inputs: InputsType, batch_size: int = 1, patch_size: int = 224, patch_stride: int =112, **kwargs):
        chunked_data = self._get_chunk_data(inputs, batch_size, patch_size, patch_stride)
        yield from map(self.collate_fn, chunked_data)

    def _get_chunk_data(self, inputs: Iterable, chunk_size: int, patch_size: int, patch_stride: int):
        inputs_iter = iter(inputs)
        while True:
            try:
                chunk_data = []
                for _ in range(chunk_size):
                    ...
                    if isinstance(inputs_, dict):
                        ...
                        large_input = self.pipeline.transforms[0](inputs_)    
                        patches, num_patch = self._slide_and_split(large_input['img'], patch_size, patch_stride)  
                        input_size = large_input['img_shape'][0]    

                        for patch in patches:       # crop to small patch
                            patch_input = copy.deepcopy(large_input) 
                            patch_input['img'] = patch
                            patch_input['img_shape'] = patch.shape[:2]
                            patch_input['ori_shape'] = (patch_size, patch_size)

                            for transform in self.pipeline.transforms[1:]:
                                patch_input = transform(patch_input)  # apply transformations in sequence

                            chunk_data = []
                            chunk_data.append((input_size, num_patch, inputs_, patch_input)) 
                            yield chunk_data
    ...
    def _slide_and_split(self, large_input, patch_size: int, patch_stride: int): 
        large_input_tensor = torch.from_numpy(large_input).permute(2, 0, 1).unsqueeze(0).float()  
        patches = F.unfold(large_input_tensor, kernel_size=(patch_size, patch_size), stride=(patch_stride, patch_stride))  
        num_patches = patches.shape[-1]    

        patches = patches.permute(0, 2, 1) 
        patches = patches.view(-1, large_input.shape[2], patch_size, patch_size)  
    ...
    def __call__(
            ...
            **kwargs) -> dict:
        '''
        4090 large image crop to small patches then process and stitch together using sliding window approach
        '''
        (
            preprocess_kwargs,
            forward_kwargs,
        ...
        inputs = self.preprocess(ori_inputs, batch_size=batch_size, patch_size=patch_size, patch_stride=patch_stride, **preprocess_kwargs)   
        results_dict = {'predictions': [], 'visualization': []}
        ...
        for large_size, num_patch, ori_imgs, data in (track(inputs, description='Inference') if self.show_progress else inputs): 
            preds = self.forward(data, **forward_kwargs)
            predictions.append(preds[0].cpu())     # append[small patch predictions] to [large image predictions]
            current_patch += 1     
            print(current_patch)
            

            if current_patch == num_patch[0]:
                print(ori_imgs)

                large_size = (large_size[0], large_size[0])   
                visualization = self.visualize(
                    ori_imgs,     
                    predictions,  
                    large_size,
    ...
    def visualize(self,
        ...
        # for single_input, prediction in zip(inputs, predictions):
        if isinstance(single_input, tuple):     
            img_bytes = mmengine.fileio.get(single_input[0])
            img = mmcv.imfrombytes(img_bytes)       
            img = img[:, :, ::-1]                  
            img_name = osp.basename(single_input[0])
        elif isinstance(single_input, np.ndarray):
            img = single_input.copy()
            img_num = str(self.num_visualized_imgs).zfill(8)
            img_name = f'{img_num}.jpg'
        ...
        self.visualizer.add_datasample_large(
            img_name,
            img,
            predictions,
            large_size,
