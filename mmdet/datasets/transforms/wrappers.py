...
# @TRANSFORMS.register_module()
class SoftMaskWrapper:
    def __init__(self, masks: List[np.ndarray], height: int, width: int):
        self.masks = np.stack(masks, axis=0)  # Shape: (num_masks, height, width)
        self.height = height
        self.width = width
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, int):  
            return SoftMaskWrapper([self.masks[idx]], self.height, self.width)
        elif isinstance(idx, slice):  
            return SoftMaskWrapper(self.masks[idx].tolist(), self.height, self.width)
    ...
    def rescale(self, target_size: tuple):
        """Resize all soft masks to a target size (height, width)."""
        from mmcv.image.geometric import imresize
        resized_masks = np.array([
                    imresize(mask, target_size) for mask in self.masks
                    ])
        return SoftMaskWrapper(resized_masks, target_size[0], target_size[1])
            

    def flip(self, flip_direction: str):
        from mmcv.image.geometric import imflip
        if flip_direction not in ["horizontal", "vertical"]:
            raise ValueError(f"Invalid flip direction: {flip_direction}. "
                             "Must be 'horizontal' or 'vertical'.")

        flipped_masks = np.array([
            imflip(mask, direction=flip_direction) for mask in self.masks
            ])
        return SoftMaskWrapper(flipped_masks, self.height, self.width)
        

    def crop(self, crop_box: List[int]):
        """Crop all soft masks using the given crop box."""
        x1, y1, x2, y2 = crop_box
        cropped_masks = [
            mask[y1:y2, x1:x2] for mask in self.masks
            ]
        return SoftMaskWrapper(cropped_masks, y2 - y1, x2 - x1)


    def get_bboxes(self, bbox_type=np.ndarray):
        ...
        return bbox_type(bboxes_array)


    def to_tensor(self):
        """Convert masks to a batched tensor."""
        import torch
        return torch.from_numpy(self.masks)


    def to_numpy(self):
        """Return the raw NumPy masks."""
        return self.masks
