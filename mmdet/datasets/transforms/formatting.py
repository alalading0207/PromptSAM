...
class PackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # teacher_model need full_img
        if 'img_resize' in results:
            img_resize = results['img_resize']
            if len(img_resize.shape) < 3:
                img_resize = np.expand_dims(img_resize, -1)
            if not img_resize.flags.c_contiguous:
                img_resize = np.ascontiguousarray(img_resize.transpose(2, 0, 1))
                img_resize = to_tensor(img_resize)
            else:
                img_resize = to_tensor(img_resize).permute(2, 0, 1).contiguous()      

            results['img_resize'] = img_resize

        if 'img_aug' in results:
            img_aug = results['img_aug']
            if len(img_aug.shape) < 3:
                img_aug = np.expand_dims(img_aug, -1)
            if not img_aug.flags.c_contiguous:
                img_aug = np.ascontiguousarray(img_aug.transpose(2, 0, 1))
                img_aug = to_tensor(img_aug)
            else:
                img_aug = to_tensor(img_aug).permute(2, 0, 1).contiguous()     
        ...
        if 'img_resize' in results:
            data_sample.img_resize = results['img_resize']

        if 'img_aug' in results:
            data_sample.img_aug = results['img_aug']
