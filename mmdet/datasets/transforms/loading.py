...
from mmdet.structures.mask import BitmapMasks, PolygonMasks, SoftmapMasks
from .wrappers import SoftMaskWrapper
import cv2
...
class LoadAnnotations(MMCV_LoadAnnotations):
   ...
   Required Keys:
    ...
    def __init__(
            self,
            with_mask: bool = False,    # True
            poly2mask: bool = True,    # True
            box_type: str = 'hbox',
            # use for semseg
    ...
    def _process_masks(self, results: dict) -> list:
        ...
        gt_ignore_flags = []
        for instance in results.get('instances', []):
            gt_mask = instance['mask']  # '/data/wuhanUIS_0519/imgs/train/1004_soft_instance/2019_5376_mask_1.npy'
            ...
            # npy
            if isinstance(gt_mask, str) and gt_mask.endswith('.npy'):
                gt_mask = np.load(gt_mask)
                # print(gt_mask.max())
                # gt_mask = cv2.resize(gt_mask, (1024,1024), interpolation=cv2.INTER_LINEAR)
                gt_mask = cv2.resize(gt_mask, results['ori_shape'], interpolation=cv2.INTER_LINEAR)
    ...
    def _load_masks(self, results: dict) -> None:
        ...
        if 'instances' in results and len(results['instances']) > 0:
            mask_state = results['instances'][0].get('mask', None)
            if isinstance(mask_state, str) and mask_state.endswith('.npy'):
        ...
        if self.poly2mask:
            ...
            # gt_masks = SoftMaskWrapper([mask for mask in gt_masks], h, w)
            gt_masks = SoftmapMasks([mask for mask in gt_masks], h, w)
