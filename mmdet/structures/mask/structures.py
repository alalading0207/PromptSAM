...
class BitmapMasks(BaseInstanceMasks):
    ...
    def rescale(self, scale, interpolation='nearest'):      # 根据比例因子调整掩码尺寸
        """See :func:`BaseInstanceMasks.rescale`."""
        if len(self.masks) == 0:
            new_w, new_h = mmcv.rescale_size((self.width, self.height), scale)  # 根据原始尺寸、比例因子  计算新尺寸  224,224->1849,1849
    ...
    def resize(self, out_shape, interpolation='nearest'):   # 将掩码调整到固定尺寸
        ...
        if len(self.masks) == 0:
            ...
            resized_masks = np.stack([
                mmcv.imresize(
                    mask, out_shape[::-1], interpolation=interpolation) # out_shape由用户指定
                for mask in self.masks
            ])
...
from mmcv.image.geometric import imrescale, imresize, imflip, impad, imtranslate, imshear, imrotate
class SoftmapMasks(BaseInstanceMasks):
    """This class represents soft masks with floating-point values.
    ...
    def __init__(self, masks, height, width):
        ...
        self.width = width
        if len(masks) == 0:
            self.masks = np.empty((0, self.height, self.width), dtype=np.float32)
        else:
            assert isinstance(masks, (list, np.ndarray))
            if isinstance(masks, list):
                assert isinstance(masks[0], np.ndarray)
                ...
                height, width = masks[0].shape  # 到时候删除

            else:
                assert masks.ndim == 3  # (N, H, W)
                _, height, width = masks.shape  # 到时候删除
    ...
    def __getitem__(self, index):
        """Index the SoftmapMask."""
        masks = self.masks[index].reshape(-1, self.height, self.width)
        return SoftmapMasks(masks, self.height, self.width)
    ...
    def rescale(self, scale, interpolation='bilinear'):
        ...
        if len(self.masks) == 0:
            new_w, new_h = mmcv.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.float32)
        else:
            rescaled_masks = np.stack([
        ...
        height, width = rescaled_masks.shape[1:]
        return SoftmapMasks(rescaled_masks, height, width)

    def resize(self, out_shape, interpolation='bilinear'):
        """Resize masks to the given out_shape."""
        if len(self.masks) == 0:
            resized_masks = np.empty((0, *out_shape), dtype=np.float32)
        else:
            resized_masks = np.stack([
                imresize(mask, out_shape[::-1], interpolation=interpolation)
                for mask in self.masks
            ])
        return SoftmapMasks(resized_masks, *out_shape)
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        return SoftmapMasks(flipped_masks, self.height, self.width)

    def pad(self, out_shape, pad_val=0.0):
        """Pad masks to the given size of (h, w)."""
        if len(self.masks) == 0:
            padded_masks = np.empty((0, *out_shape), dtype=np.float32)
        else:
            padded_masks = np.stack([
                impad(mask, shape=out_shape, pad_val=pad_val)
                for mask in self.masks
            ])
        return SoftmapMasks(padded_masks, *out_shape)
    ...
    def crop(self, bbox):
        ...
        if len(self.masks) == 0:
            cropped_masks = np.empty((0, h, w), dtype=np.float32)
        else:
            cropped_masks = self.masks[:, y1:y1 + h, x1:x1 + w]
        return SoftmapMasks(cropped_masks, h, w)
    
    def get_bboxes(self, dst_type='hbb', threshold=0):
        """Compute bounding boxes from soft masks.

        Args:
            dst_type (str): Type of the output bounding boxes (e.g., 'hbb').
            threshold (float): Threshold for binary mask conversion.

        Returns:
            BaseBoxes: Generated bounding boxes.
        """
        from ..bbox import get_box_type

        soft = self.masks
        self.masks = soft > threshold
        _, box_type_cls = get_box_type(dst_type)
        bbox = box_type_cls.from_instance_masks(self)
        # 还原
        self.masks = soft
        return bbox
    ...
    def crop_and_resize(self,
        ...
        """Crop and resize masks by the given bboxes."""
        if len(self.masks) == 0:
            empty_masks = np.empty((0, *out_shape), dtype=np.float32)
            return SoftmapMasks(empty_masks, *out_shape)
    ...
    def expand(self, expanded_h, expanded_w, top, left):
        """Expand masks by adding padding around them."""
        if len(self.masks) == 0:
            expanded_mask = np.empty((0, expanded_h, expanded_w),
                                     dtype=np.float32)
        else:
            ...
            expanded_mask[:, top:top + self.height,
                          left:left + self.width] = self.masks
        return SoftmapMasks(expanded_mask, expanded_h, expanded_w)
    ...
    def translate(self,
        ...
        """
        if len(self.masks) == 0:
            translated_masks = np.empty((0, *out_shape), dtype=np.float32)
        else:
            masks = self.masks
            if masks.shape[-2:] != out_shape:
                empty_masks = np.zeros((masks.shape[0], *out_shape),
            ...
            translated_masks = translated_masks.transpose(
                (2, 0, 1)).astype(self.masks.dtype)
        return SoftmapMasks(translated_masks, *out_shape)
    ...
    def shear(self,
              ...
              border_value=0.0,
              interpolation='bilinear'):
        """Shear the SoftmapMasks."""

        if len(self.masks) == 0:
            sheared_masks = np.empty((0, *out_shape), dtype=np.float32)
        else:
            ...
            sheared_masks = sheared_masks.transpose(
                (2, 0, 1)).astype(self.masks.dtype)
        return SoftmapMasks(sheared_masks, *out_shape)
    ...
    def rotate(self,
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = rotated_masks.transpose(
                (2, 0, 1)).astype(self.masks.dtype)
        return SoftmapMasks(rotated_masks, *out_shape)
    ...
    def random(cls,
        ...
        masks = rng.random((num_masks, height, width), dtype=dtype)
        self = cls(masks, height=height, width=width)
        return self

    @classmethod
    def cat(cls, masks):
        """Concatenate multiple SoftmapMask instances."""
        assert all(isinstance(m, cls) for m in masks)
        concatenated = np.concatenate([m.masks for m in masks], axis=0)
        return cls(concatenated, masks[0].height, masks[0].width)
